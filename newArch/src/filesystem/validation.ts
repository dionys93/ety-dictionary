...
// IMPURE: Runtime I/O - reads filesystem state
// Returns DataContent directly instead of ValidatedFile | ValidatedDirectory
export async function validateFileSystemEntity(
  path: string
  ...
  try {
    ...
    if (stats.isDirectory()) {
      ...
      // Return as DataContent
      return success<DataContent>(validatedDir)
    }
    ...
    if (stats.isFile()) {
      ...
      // Return as DataContent
      return success<DataContent>(validatedFile)
    }
...
export async function validateDataTextDirectory(
  ...
  return flatMap(validationResult, function(entity) {
    ...
    // Only after both validations pass do we brand it as DataTextDir
    // Need to access the underlying path from the ValidatedDirectory
    const validatedDir = entity as ValidatedDirectory
    return success(validatedDir.path as DataTextDir)
  })
}