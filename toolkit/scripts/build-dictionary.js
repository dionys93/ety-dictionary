 ...
 * @property {string} ing 
 * @property {string} pos 
 * @property {Object|null} conj 
 */

/**
 * The immutable state flowing through the build pipeline.
...
const normalizeConjugations = (baseEng, rawConj, posCategories) => {
    ...
    if (baseEng === 'do') {
        if (posCategories.includes('Auxiliary')) {
            return {
                explicit: {
                    does: rawConj[0], did: rawConj[1], "don't": rawConj[2], 
                    "doesn't": rawConj[3], "didn't": rawConj[4]
                }
            };
        }
        return {
            explicit: {
                does: rawConj[0], did: rawConj[1], done: rawConj[2], doing: rawConj[3], 
                "don't": rawConj[4], "doesn't": rawConj[5], "didn't": rawConj[6]
            }
        };
    }
    if (baseEng === 'have') {
...
/**
 * RawRow -> Either<string, ProcessingState>
 * @param {RawRow} row 
 * @returns {Either<string, ProcessingState>}
 */
const validateAndClean = (row) => {
    if (!row || !row.me_word || !row.inglisce_word) return Failure('Missing core words');
    ...
    return Success({
        baseEng, baseIng, posCategories, conjugations: cleanConjugations, mappings: []
    });
};

/**
 * Either<string, ProcessingState> -> Either<string, ProcessingState>
 * @param {Either<string, ProcessingState>} result 
 * @returns {Either<string, ProcessingState>}
 */
const generateMappings = (result) => {
    if (result.status === 'skipped') return result;
