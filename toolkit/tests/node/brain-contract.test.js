...
const { checks, warnings, compiledCount, datasetCount } = runContract();
...
describe('awk → Node Schema Contract (golden JSONL through buildBrain)', () => {
    ...
    it('surfaces known DATA issues as warnings (fix the .txt files, then refreeze)', () => {
        for (const w of warnings) console.warn(`⚠️  ${w}`);
        // Warnings are data problems, not code problems — never fail the build.
