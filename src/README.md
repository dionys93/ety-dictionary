...
**Dependencies:** The toolkit includes a boot-time check for `jq`. It is strictly required for the `etym-export` and `etym-summarize` functions to compile JSON objects. If it is missing, the script will prompt you to install it automatically via `apt` (Linux) or `brew` (macOS).
...
* **Usage:** `etym-create-histories` (processes the entire dictionary).
* **Options:**
...
* **Output:** Creates individual text files in the `histories/` directory named by word and part of speech (e.g., `adroit_adj.txt`). 
...
* **Options:**
  * `--json`: Export the summary as a structured JSON object.
  * `-o, --out <file>`: Write the summary directly to a specified text or JSON file.
* **Result:** Generates a ranked frequency report of Parts of Speech and Language Origins, mapped to their full names (e.g., `    75 | verb (v)`).
