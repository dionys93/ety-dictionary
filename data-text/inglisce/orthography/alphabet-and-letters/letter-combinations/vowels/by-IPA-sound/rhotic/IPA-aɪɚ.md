# IPA /aɪər/ — "FIRE" DIPHTHONG

## Sound Overview

| Feature | Value |
|----------|--------|
| IPA | /aɪər/ |
| Description | Low-front to high-front closing diphthong + rhotacization |
| Example in Modern English | fire, hire, desire |
| Frequency | Moderately common; especially in Latinate verb stems |

---

## Spelling Patterns

| Category | Spellings |
|-----------|------------|
| Primary | Î (in ÎRE stems) |
| Noun plural | -ÎARS |
| Verb infinitive | -ÎER |
| Progressive | -ÎARING |
| Exception | OÎ (W-initial words) |

'*' Represents no change in spelling between Modern (Websterian) English and Inglisce

---

## Core Distribution Rules

### 1. ÎRE = /aɪər/ (Nouns)

The default spelling for /aɪər/ in nouns. Plurals follow the -ÎARS pattern.

| Modern | Inglisce |
|--------|----------|
| choir, choirs | coîre, coîars |
| desire, desires | desîre, desîars |
| fire, fires | fuîre, fuîars |
| hire | hîre, hîars |
| quire, quires | quîre, quîars |
| spire, spires | spîre, spîars |
| tire, tires (wheel) | tîre, tîars |
| wire, wires | uîre, uîars |
| iron, irons | îarn, îarns |

---

### 2. ÎER = /aɪər/ (Verbs)

Verbs follow a consistent conjugation pattern. The infinitive uses -ÎER; the present stem restores -ÎRE.

| Infinitive | Present | Past | Progressive |
|------------|---------|------|-------------|
| to acquîer | acquîre(s | acquîred | acquîaring |
| to admîer | admîre(s | admîred | admîaring |
| to attîer | attîre(s | attîred | attîaring |
| to desîer | desîre(s | desîred | desîaring |
| to expîer | expîre(s | expîred | expîaring |
| to fuîer | fuîre(s | fuîred | fuîaring |
| to hîer | hîre(s | hîred | hîaring |
| to inquîer | inquîre(s | inquîred | inquîaring |
| to inspîer | inspîre(s | inspîred | inspîaring |
| to requîer | requîre(s | requîred | requîaring |
| to retîer | retîre(s | retîred | retîaring |
| to spîer | spîre(s | spîred | spîaring |
| to uîer | uîre(s | uîred | uîaring |
| to tîore (to exhaust) | tîore(s | tîored | tîoring |

---

### 3. Derived Nouns

Verbs with /aɪər/ stems form derived nouns via Latin/French endings, often losing the diphthong.

| Inglisce Verb | Inglisce Noun |
|------|----------|
| to admîer | admirâcion |
| to inspîer | inspirâcion |
| to expîer | expirâcion |
| to acquîer | acquïsicion |
| to requîer | requîrement |
| to retîer | retîrement |

---

## Cross-References

| Topic | Relationship |
|---------|--------------|
| /aɪ/ | Compare /aɪər/ with plain /aɪ/; rhotacization distinguishes the two |
| No W rule | W-initial words use OÎ; see U spelling file |
| Verb conjugation | -ÎER infinitive / -ÎRE present pattern; compare with other verb classes |
| Derived nouns | -ÂCION and -MENT endings; compare with complícion, requîrement patterns |
